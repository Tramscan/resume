...
% Reduce overall vertical spacing throughout the document
\usepackage{setspace}
\setstretch{1.0}  % Sets line spacing to single (1.0). Decrease for less space, increase for more.

% Define a command for section headings with reduced spacing
\newcommand{\sectionheading}[1]{%
  \vspace{0.6em}%                         % Space above the heading text
  {\large\textbf{\MakeUppercase{#1}}}
  %\\[-0.8em]%                             % Space between text and line
  \rule{\textwidth}{0.1pt}%               % Horizontal line
  %\\[-1.0em]%                             % Space below the line
}
...
}  % No explicit vertical spacing here, but affects space between entries

% Reduce space between items in lists
\usepackage{enumitem}
\setlist{noitemsep, topsep=0pt, parsep=0pt, partopsep=0pt}  % Removes extra spacing in lists
...
\vspace{0.5em}  % Add vertical space between name and contact info
